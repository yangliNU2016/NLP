...
N1 = 0.0
N2 = 0.0
for x in range(0, counts.shape[0]):
	for y in range(0, counts.shape[1]):
		if counts[x, y] != 0:
			N += counts[x, y]
		if counts[x, y] == 1:
			N1 += 1.0
...
for x in range(0, probs.shape[0]):
	for y in range(0, probs.shape[1]):
		if probs[x, y] == 0:
			probs[x, y] = p0
		if probs[x, y] == 1:
			probs[x, y] = p1

probs = normalize(probs, norm='l1', axis=0)
...
target1 = open('Good_Turing_smoothed_eval.txt', 'w')
